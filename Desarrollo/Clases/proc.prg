			...
			IF lok=6		 	
	            ...
	            *Oavisar.proceso('S','Aguarde un instante, recuperando '+ALLTRIM(STR(Csridgestion.id)),.t.,lncantreg,lniterar)				     										      
				...
				IF lbdescargado
					...
					*Alamacenamos la copia
					lcCopia = ADDBS(lcCurdir) + "copias"
					*OAVISAR.usuario(lcCopia)
...
*!*	TRY 
	DO case
		CASE LcDataSourceType='ADO' OR LcDataSourceType='ODBC'
		    ...
		    Catch To loCatchErr
				=Oavisar.proceso('N')	
				=Oavisar.programador('002-La conexión a la Base de Datos a fallado'+CHR(13);
		...
		CASE LcDataSourceType='NATIVE'
			...
			Catch To loCatchErr
				=Oavisar.proceso('N')	
				=Oavisar.usuario('003-La conexión a la Base de Datos a fallado'+CHR(13);
								+CHR(13)+lcConectionString,0)
				lok = .f.
			ENDTRY 	
