...
FUNCTION GoogleStreetViewUnicAPI
...
<html>
  <head>
    ...
    <script async defer src="https://maps.googleapis.com/maps/api/js?key=<<GoogleMapsKeyAPI>>&callback=initMap"
...
IF !DIRECTORY(lcRuta)
	MKDIR &lcRuta
ENDIF 
lcFile = ADDBS(lcRuta)+"geoloca_"+tcDestination+".html"
...
lcRouteParameters = STRTRAN(UPPER(ALLTRIM(tcDestination)), " ", "%20") + "&z9"
lcFullURL = "https://maps.googleapis.com/maps/api/geocode/xml?address="+lcRouteParameters+",+CA&key="+GoogleMapsKeyAPI
...
lcAddress = STREXTRACT(lcResponse, "<formatted_address>", "</formatted_address>")
lcLocation = STREXTRACT(lcResponse, "<location>", "</location>")
lcLatitud = STREXTRACT(lcLocation, "<lat>", "</lat>")
lcLongitud = STREXTRACT(lcLocation, "<lng>", "</lng>")
...
* Build up the full URL with the required parameters
lcFullURL = "https://maps.googleapis.com/maps/api/geocode/xml?latlng="+clatitud+","+clongitud+"&sensor=false&v=3.22&key="+GoogleMapsKeyAPI
...
*-----------------------------------
*-----------------------------------
*GOOGLECOORDS3 pasando la cadena de coordenadas COORDS y la cadena de marcadores y info windows CMARKS
*Crea una ruta entre las coordenadas, elimina los marcadores del tipo A,B,C...
*agrega marcadores comunes en cada direccion que poseen una InfoWindow con Nombre y Direccion
