...
cSepDec = IIF(PCOUNT()<3,'.',cSepDec)

cValor = TRANSFORM(nValor,cMascara)
IF cSepDec<>'.'
...
ENDFUNC
*-----------------------------
FUNCTION DeleteFileGM(lcFile,lForce)
lForce = IIF(PCOUNT()<1,.t.,lForce)
LOCAL loScript, loFile
IF FILE(lcFile)
	loScript = CREATEOBJECT("Scripting.FileSystemObject")
	loFile = loScript.DeleteFile(lcFile,lForce)
	IF loFile=.f.
		oavisar.usuario('El archivo'+CHR(13)+lcFile+CHR(13)+'No es posible borrarlo automaticamente.'+CHR(13)+'Cierre Aplicacion/Servicios de Excel')
		RETURN .f.
	ENDIF 
ENDIF 
...
FUNCTION CreateFileGM(lcFile,lForce)
lForce = IIF(PCOUNT()<1,.t.,lForce)
LOCAL loScript, loFile

loScript = CREATEOBJECT("Scripting.FileSystemObject")
loFile = loScript.CreateTextFile(lcFile,lForce)

RELEASE loScript
RETURN loFile
*---------------
FUNCTION CreateFolderGM(lcFolderDir,lcFolderNew)

lcFolderNew = IIF(PCOUNT()<2,'',lcFolderNew)
LOCAL loScript

loScript = CREATEOBJECT("Scripting.FileSystemObject")
lcFolderAux = ADDBS(lcFolderDir)+lcFolderNew
IF loScript.FolderExists(lcFolderAux)
	RETURN .t.
ELSE
	IF EMPTY(lcFolderNew)
		*stop()
		&&No viene carpeta nueva, entonces creamos solo el directorio
		lcFolderDir = LEFT(lcFolderDir,LEN(lcFolderDir) - IIF(RIGHT(lcFolderDir,1)='\',1,0))
		i = OCCURS('\',lcFolderDir )
		i = AT('\',lcFolderDir,i)
		lcFolderNew = SUBSTR(lcFolderDir,i+1)
		lcFolderDir = LEFT(lcFolderDir,i)
	ENDIF 
	*loFolder = loScript.GetFolder(folderspec)
	lcFolder = lcFolderDir
	lcFolderNew = ADDBS(lcFolderNew) &&Le agrego el ultimo para siempre tener '\'
*!*		DO WHILE NOT EMPTY(lcFolderNew)
*!*			i = AT('\',lcFolderNew)
*!*			lcFolder = ADDBS(lcFolder)  + STRTRAN(LEFT(lcFolderNew,i),'\','')
*!*			lcFolderNew = SUBSTR(lcFolderNew,i+1)
*!*			IF NOT loScript.FolderExists(lcFolder)
*!*				loScript.CreateFolder(lcFolder)
*!*			ENDIF 
*!*		ENDDO 
	lcFolder = ADDBS(lcFolder)+lcFolderNew 
	lcFolderAux = lcFolder
	lcFolder = ''
	i=99
	*stop()
	DO WHILE i<>0
		i = AT('\',lcFolderAux)
		IF i > 0
			lcFolder = lcFolder + LEFT(lcFolderAux,i-1)
			IF NOT loScript.FolderExists(lcFolder)
				loScript.CreateFolder(lcFolder)
			ENDIF 
			lcFolderAux = SUBSTR(lcFolderAux,i+1)
			lcFolder = lcFolder + "\"
		ENDIF 
	ENDDO  
ENDIF 
...
*----------------
*---------------
FUNCTION FolderGM(lcFolderDir)

LOCAL loScript,loFolder

loScript = CREATEOBJECT("Scripting.FileSystemObject")
lcFolderAux = ADDBS(lcFolderDir)
IF loScript.FolderExists(lcFolderDir)
	loFolder = loScript.GetFolder(lcFolderDir)
...
RETURN loFolder
*---------------
FUNCTION FolderDeleteGM(lcFolderDir)
...
lcFolderAux = ADDBS(lcFolderDir)
IF loScript.FolderExists(lcFolderDir)
	loFolder = loScript.DeleteFolder(lcFolderDir)
	*= loFolder.Delete(lcFolderDir,.t.)
	IF loFolder =.f.
		oavisar.usuario('El directorio'+CHR(13)+lcFile+CHR(13)+'No es posible borrarlo automaticamente.'+CHR(13)+'')
		RETURN .f.
	ENDIF 
ENDIF 
...
RETURN .t.
*---------------------------------------------------
FUNCTION SumarLetras
LPARAMETERS cLetra, nNext
LOCAL cLetraSig, nAscii

cCadena = 'ABCDEFGHIJKLMNÑOPQRSTUVWXYZ'
i = AT(cLetra,cCadena)
cLetraSig = ""
IF i + nNext <= LEN(cCadena)
	cLetraSig  = SUBSTR(cCadena,i+nNext,1)
ENDIF 
...
*---------------------------------------------------
FUNCTION ExplorerFileShell
LPARAMETERS lcfile,nTipo

IF NOT directory(lcFile)
	oavisar.usuario('No se encontrol el destino'+CHR(13)+CHR(13)+'Destino: '+lcFile)
	RETURN 
ENDIF 
NTipo = 1 &&IIF(PCOUNT()<2,3,nTipo)
...
*lcPDFfile = "C:\ebooks\web\visual c.pdf"
* Open PDF file
kk=Shellexecute(0,"Explore", lcfile, "","",nTipo)
*--------------------------------------------------
FUNCTION ReccountFull
PARAMETERS cAlias
LOCAL nRegCount

SELECT(cAlias)
COUNT FOR NOT DELETED() TO nRegCount

RETURN nRegCount


FUNCTION CursorAuxiliar
PARAMETERS cAlias, cAliasAux
IF NOT USED(cAlias)
	RETURN 
ENDIF 
cAliasAux = IIF(PCOUNT()<2,'',cAliasAux)
IF LEN(TRIM(cAliasAux))=0
	cAliasAux = 'CsrAux'+SUBSTR(cAlias,4)
ENDIF 
SELECT * FROM (cAlias) WHERE RECNO()< 1 INTO CURSOR &cAliasAux READWRITE 
...
LPARAMETERS cReport, cProperty,cValor,lSilent

lSilent = IIF(PCOUNT()<4,.f.,lSilent)

LOCAL cMemo,cMemoNew ,cCopia1,cCopia2
*'PAPERLENGTH','9999'
IF NOT FILE(cReport)
	RETURN 
ENDIF 
IF USED('DataReporte1')
	USE IN DataReporte1
ENDIF 
*!*	USE (cReport) ALIAS DataReporte
USE (cReport) ALIAS DataReporte1
SELECT DataReporte1
GO TOP 
cMemo = DataReporte1.Expr
...
IF i#0
	...
	cCopia2 = SUBSTR(SUBSTR(cMemo,i),17)
	 ...
	 ENDIF 
	 replace Expr WITH cMemoNew IN DataReporte1
ELSE
...
IF oConfigTermi.LauncherSQL='FALSE'
	lcRespuesta  = HayVersionExeLocal(lcExe,nidprograma)
	RETURN lcRespuesta 
ENDIF 
...
=CargarXML('ParaConfig.xml','CsrVersConfig')
LOCAL cPathUpdate
STORE "" TO cPathUpdate

SELECT CsrVersConfig
GO TOP 
DO WHILE not EOF() AND LEN(cPathUpdate)=0
	SCATTER NAME OscCursor
	IF OscCursor.nrosistema = nidprograma 
		lcExeName      = OscCursor.Path    && guardo el nombre del ejecutable
		j	= LEN(lcExeName)
		lcCurdir	= lcExeName
		lFin	= .f.
		cSistema = ""
		DO WHILE LEN(lcCurdir)<> 0 AND NOT lFin			
			cSistema = RIGHT(lcCurDir,1)+cSistema
			lcCurdir	= SUBSTR(lcCurdir,1,LEN(lcCurdir)-1)
			IF RIGHT(lcCurdir,1)='\'
				lFin = .t.
			ENDIF 
		ENDDO 
		IF cSistema = lcExe
			cPathUpdate = ADDBS(DefaultVar('OscCursor.PathUpdateEXE','','C'))
		ENDIF 
	ENDIF 
...
IF USED("CsrVersConfig")
	USE IN CsrVersConfig
ENDIF
	
lcCurdir = TRIM(SYS(5))+TRIM(CURDIR())

STORE "" TO lcfechasis,lchorasis,lchoraExe,lcfechaexe,lcRespuesta 

*STOP()
j = Adir(lCarray, cPathUpdate +'*',"H")
IF j>0
	ldfechaini = stod('19000101')
	FOR i=1 TO j
		IF (lcArray[i,3]) > ldfechaini AND ALLTRIM(lcArray[i,1]) = UPPER(lcExe)
			ldfechaini = lcArray[i,3]
			lcfechasis = DTOS(ldfechaini )
			lchorasis  = lcarray[i,4]
		ENDIF 
	ENDFOR 

	lcExe          = lcCurdir+lcExe     && guardo el nombre del ejecutable
	...
	lcMensajePrioridad = ""
    	...
    	lcfechaexe = DTOS(TTOD(CTOT(stoc(lcfechasis)) - 200))
	lchoraexe  = lchorasis
	IF x=1
		lcfechaexe = DTOS(lcArray[1,3])
		lchoraexe   = lcarray[1,4]
...
oConfigTermi.AddProperty('VersionWindows','64')
oConfigTermi.AddProperty('LauncherSQL','TRUE')
oConfigTermi.AddProperty('Desarrollo','FALSE')
oConfigTermi.AddProperty('EmailOld','FALSE')
oConfigTermi.AddProperty('ControlSkin','FALSE')
	...
	DO WHILE !FEOF(Arc)
		*lcActDato			=FREAD(Arc,LenRegistro)
		...
		DO case
			...
			CASE lclabel="VERSIONWINDOWS"
				oConfigTermi.VersionWindows=  ALLTRIM(SUBSTR(lcActDato,i))	
			CASE lclabel="LAUNCHERSQL"
				oConfigTermi.LauncherSQL=  ALLTRIM(SUBSTR(lcActDato,i))	
			CASE lclabel="DESARROLLO"
				oConfigTermi.desarrollo=  ALLTRIM(SUBSTR(lcActDato,i))	
			CASE lclabel="EMAILOLD"
				oConfigTermi.emailold=  ALLTRIM(SUBSTR(lcActDato,i))	
			CASE lclabel="CONTROLSKIN"
				oConfigTermi.controlskin=  ALLTRIM(SUBSTR(lcActDato,i))	
			CASE lclabel="FACVTA_FORECOLOR_LABEL"
				oConfigTermi.facvta_forecolor_label=  ALLTRIM(SUBSTR(lcActDato,i))	
...
*!*			XX= FPUTS(Arc,lclabel,lenregistro)	
*!*			lclabel="[VersionWindows]" + oConfigTermi.VersionWindows
*!*			XX= FPUTS(Arc,lclabel,lenregistro)	
*!*			lclabel="[LauncherSQL]" + oConfigTermi.LauncherSQL
*!*			XX= FPUTS(Arc,lclabel,lenregistro)		
*!*			lclabel="[Desarrollo]" + oConfigTermi.desarrollo
*!*			XX= FPUTS(Arc,lclabel,lenregistro)	
*!*			lclabel="[EmailOld]" + oConfigTermi.emailold
*!*			XX= FPUTS(Arc,lclabel,lenregistro)	
*!*			lclabel="[ControlSkin]" + oConfigTermi.controlskin
*!*			XX= FPUTS(Arc,lclabel,lenregistro)	
*!*			lclabel="[facvta_forecolor_label]" + oConfigTermi.facvta_forecolor_label
*!*			XX= FPUTS(Arc,lclabel,lenregistro)	
*!*			lclabel="[facvta_forecolor_textbox]" + oConfigTermi.facvta_forecolor_textbox
*!*			XX= FPUTS(Arc,lclabel,lenregistro)				
*!*		ENDIF 	
*!*		FCLOSE(Arc)
*!*	ENDIF 
...
WITH loObjeto
	...
	FOR i=1 TO lnGrupos
		...
		ObjGrupo.AddProperty('HeaderFirstVisible',.f.)
...
WITH loObjeto
	...
	FOR i=1 TO lnColumnas
		...
		ObjColumna.AddProperty('ColorIndex',14277082)
		
		ObjColumna.AddProperty('HeaderFirst',"")
		ObjColumna.AddProperty('EsCombinada',.f.) &&Siempre es verdadero. Si se quiere Que el ancho funcione
...
lstOCX(14,1) ="CsFoxySmtp.dll"
lstOCX(14,2) ="CsFoxySmtp.dll"
