...
PROCEDURE AddReport(tcReport, tcClauses, tcAlias)
	*stop()
	...
	*stop()	
	lcExtension=RTRIM(UPPER(JUSTEXT(tcReport)))
	...
	ELSE
		...
		* extensiones de archivos son de 3
		lcExtension2=STUFF(lcExtension, 3, 1, [T])
	ENDIF
	...
	* Retrieve the FRX, FRT, LBX, LBT, ??X, ??T files...from the EXE
	...
	This._oNames.Add(JUSTSTEM(JUSTFNAME(tcReport)))
