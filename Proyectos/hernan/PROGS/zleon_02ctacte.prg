...
llok = CargarTabla('leon','Ctacte',.t.)
llok = CargarTabla('leon','Provincia')
llok = CargarTabla('leon','CateIbRn',.t.)
llok = CargarTabla('leon','CateIbRNg',.t.)
llok = CargarTabla('leon','TipoIva')
llok = CargarTabla('leon','CateCtacte',.t.)
llok = CargarTabla('leon','Barrio',.t.)
...
SCAN 
		...
		SELECT CsrDeudor
		lntipoiva = VAL(str(11,2)+strzero(tipocuit,lntamiva))
		IF lntipoiva=VAL(str(11,2)+strzero(7,lntamiva))
			   lntipoiva = VAL(str(11,2)+strzero(5,lntamiva))
		ENDIF 
		IF lntipoiva=VAL(str(11,2)+strzero(3,lntamiva))
			lncuit=''
		ENDIF
		...
		lcnombre = NombreNi(ALLTRIM(UPPER(CsrDeudor.nombre)))
		  ...
		  ,lnidlocalidad,lnidprovincia,CsrDeudor.telefono,lntipoiva,CsrDeudor.cuit,lnidcategoria,0,0;
