...
ENDIF 
TEXT TO lccmd TEXTMERGE NOSHOW 
SELECT CsrProvCtaCon.*,CsrProvincia.numero FROM ProvCtaCon as CsrProvCtaCon
left join PlanCue as CsrPlanCue on CsrProvCtaCon.idctarete = CsrPlancue.id
inner join provincia as CsrProvincia on CsrProvCtacon.idprovincia = CsrProvincia.id
...
**OR tipocomp=5
SELECT distinct cliente+10000 as cliente,fecha_fac,numcomp;
...
SCAN 
	...
	*lncliente = CsrAnterior.cliente 
	lncliente = CsrProv_SaldoAnt.cliente
	IF lncliente=10000+118
		stop()
	ENDIF 
	*SELECT CsrProv_Saldoant
	SELECT CsrAnterior
	LOCATE FOR cliente != lnCliente
	IF cliente != lnCliente
		APPEND BLANK IN Csranterior
		replace cliente WITH lnCliente,importe WITH importe + (CsrProv_Saldoant.saldo_ant *(-1))
	ENDIF 	
	*SELECT CsrAnterior
	replace saldo WITH (saldo + (CsrProv_Saldoant.saldo_ant *(-1))) &&Sumamos  pq el saldo_ant postivo es un credto.
...
SCAN FOR !EOF('CsrAnterior')
	...
	IF  ALLTRIM(cnumero)=ALLTRIM(STR(CsrAnterior.cliente))
		lnsigno=-1
		replace saldo WITH CsrCtacte.saldo + CsrAnterior.Saldo + CsrCtacte.SaldoAnt IN CsrCtacte
		replace saldoant WITH 0 IN CsrCtacte
...
*!*				lnidprov = CsrParaVario.idorigen
			...
			lnCodprov = IIF(CsrAnterior.TotalPBBA!=0,2,22)
			...
			SELECT CsrProvCtaCon
			LOCATE FOR numero = lnCodProv
			IF numero = lnCodProv AND CsrProvCtacon.idctaperce !=0
