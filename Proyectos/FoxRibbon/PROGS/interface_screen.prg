...
llTitleBarWin = .F. && Activar Barra de título Windows. Si FoxRibbon ya tiene una barra propia, la barra Windows se desactivará igualmente

llFunction		  = .T. && Activar CamfFunction -> Function 
llSidebar		  = .T. && Activar Sidebar
llGradient		  = .T. && Activar Gradiente

llTitleBar		  = .T. && Activar TitleBar.    Desactivado, pues lo activa Ribbon en esta configuración
llStatusBar		  = .T. && Activar StatusBar.   Desactivado, pues lo activa Ribbon en esta configuración
llRibbon		  = .T. && Activar Ribbon
llStartButton 	  = .T. && Activar StartButton. Desactivado, pues lo activa Ribbon en esta configuración
llTitleBar_Little = .F. && Utilizar barra de título pequeña. Normalmente se utiliza en Ribbon cuando no tenemos una barra de título
llLanguage		  = .F. && Activar cambio de idiomas. Está desactivado porque el objeto ya se ha creado en Ribbon y Ribbonc

*- Los objetos se tienen que añadir en el estricto orden que tiene esta configuración
WITH _SCREEN
	llOldLock = .LOCKSCREEN
	.LOCKSCREEN = .T. && Para no ver modificaciones hasta el final cuando .LOCKSCREEN = llOldLock
	* .function1.LOCKSCREEN(.T.) && Si se utiliza poner después de crear el objeto function1, es decir, 2 lineas mas abajo
	.MINWIDTH = 140
	.MINHEIGHT = 50
*---
	IF llTitleBarWin .AND. llTitleBar = .F. .AND. llTitleBar_Little = .F. && Si no tenemos otras barras de título
		.TitleBar = 1 && 0 - Desactivamos barra de título.
		.Caption="FoxRibbon Camf trabajando en la ventana principal de Visual FoxPro - Mode: Screen" && Texto en la barra de título de Visual FoxPro
	ENDIF
*-
...
*-
    IF llSidebar
       ...
       *Al final está la llamada a las funciones de centrado 
    ENDIF
*-
    IF llGradient
...
*- Camf_Engine
    IF "CAMFENGINE" $ SET("CLASS") 
    	...
    	ENDIF	
       .NEWOBJECT('camf_engine1', 'camf_engine') &&Es el motor FoxRibbon diseñado por Camf 
    ENDIF									  	 
*-
    IF llSidebar
			.Camf_Engine1._mySideBar = .o_mysidebar1.NAME	&& Añadimos código al motor para saber que tenemos un SideBar
    ENDIF
*-
    IF llTitleBar
    	IF  tlCamf_Ribbonc
	    	...
	    	*No se activa con Ribbon ya que que se activa en YourTitlebar en la clase my_ribbon_screen
			.Caption="FoxRibbon Camf trabajando en la ventana principal de Visual FoxPro - Ribbonc - Mode: Screen" && Texto en la barra de título
			IF VARTYPE(_SCREEN._My_TitleBar)="O"
    			_SCREEN.removeobject('_My_TitleBar')
 		   	ENDIF	
			.NEWOBJECT("_My_TitleBar", "_c_titlebar_designer")	 && Se puede crear aquí o en Ribbon
			*.Camf_Engine1._MyTitleBar = ._my_titleBar.Name && Añadimos código al motor para saber que tenemos un TitleBar
			*.WindowState = 2
			*.ribbon1.Top = 30 && .Top donde aparece Ribbon ya que tiene que ser desplazado hacia abajo
			**.ribbon1.Anchor = 5 &&Sólo horizontal, puede ser interesante
...
*-
    IF llStatusBar
    	IF VARTYPE(_SCREEN._My_StatusBar)="O"
 	   	...
 	   	ENDIF	
       .NEWOBJECT("_My_StatusBar", "_c_StatusBar") && Se puede crear aquí o en Ribbon
    ENDIF
*-
    IF llRibbon
		...
		IF ! tlCamf_Ribbonc	
			...
			*.Camf_Engine1.ReDraw() && No es necesario aquí porque para el ejemplo y centrar _language se ha puesto una línea al final 
		ELSE
			.NEWOBJECT("ribbon1", "my_ribbonc_screen", "SampleRibbon.vcx")
			IF llTitleBar	&& Si creamos el título aquí, colocamos RibbonC debajo del título
				.RIBBON1.top = 30	&& Sólo para _ribbonc_screen. Con Ribbon debemos poner el ._Titlebar en Ribbon.YourTitlebar
			ENDIF
		ENDIF
		.ribbon1.visible = .T.	&& modificado en Ribbon.Init. No en Ribbonc
	ENDIF
*-
    IF llStartButton	&& Tiene que ir después de llRibbon
		IF VARTYPE(_SCREEN._My_StartButton)="O"
   			_SCREEN.removeobject('_My_StartButton')
   		ENDIF	
       .NEWOBJECT("_My_StartButton", "_My_StartButton")	&& Se puede crear aquí o en Ribbon
    ENDIF
*-
...
*-
	...
	* ENDWITH
    ENDIF
*-
    IF llSideBar	&& Por Camf para centrar o_mysidebar1. Tenemos que configurarlo desde aquí y en modo FNS tiene su configuración en _mysidebar 
...
*  .LOCKSCREEN = llOldLock && Ahora podemos ver todo de un pantallazo     .refresh()	&& Necesario. De lo contrario no se actualiza _language
    ...
    IF llRibbon 
       .Camf_Engine1.ReDraw() 				&& No es necesario con Ribbonc a no ser por el ejemplo y centrar _language. Si es necesario con Ribbon y se ha anulado 
										&& su linea correspondiente porque se ha bajado aquí a causa del ejemplo
    ENDIF
...
*	.alwaysOnTop = .F. && VentanaTopMost(1) abajo también corrige el problema. Usar uno de los dos
