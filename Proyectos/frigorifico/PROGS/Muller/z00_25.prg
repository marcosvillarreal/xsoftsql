...
FUNCTION LeerClientes_25(cArchivo)
	...
	DO WHILE lnCamposLeidos<4
		...
		DO WHILE i + j <= lnCantCampo &&Campos de CsrArti + 1
			...
			*lcIDJ			= UPPER(LimpiarCadena(IIF(j + i=1,lcCadena,lcIdJ)))
			...
			lcProveedor		= UPPER(LimpiarCadena(IIF(j + i=3,lcCadena,lcProveedor))) &&Marca
			lcRubro			= UPPER(LimpiarCadena(IIF(j + i=4,lcCadena,lcRubro)))
			lcPesoKilos		= UPPER(LimpiarCadena(IIF(j + i=6,lcCadena,lcPesoKilos)))
			lcMerma			= UPPER(LimpiarCadena(IIF(j + i=7,lcCadena,lcMerma)))
			lcAlicuota		= "21"
			
			lcCosto			= IIF(j + i=8,lcCadena,lcCosto)
			lcLista1		= IIF(j + i=9,lcCadena,lcLista1) &&Utilidad
			lcLista2		= IIF(j + i=10,lcCadena,lcLista2) &&Precio s/iva
			lcAlicuota		= IIF(j + i=11,lcCadena,lcAlicuota) 
...
FUNCTION LeerCartera_25(cArchivo)

CREATE CURSOR CsrLista (deta01 c(250),deta02 c(250),deta03 c(250) )

CREATE CURSOR CsrCartera (Fecha c(10),Numero c(20),femision c(10),Importe c(20);
			,cliente c(100),titular c(100),cuit c(20),banco c(50),afecta c(20);
			,localidad c(100))
...
*!*		IF AT(lcDelimitador,deta01)=lnPrimeraOcurrencia
		leiunarticulo = .t.
		STORE "" TO lcAcarreo,lcIdJ
		STORE "" TO lcFecha ,lcNumero ,lcfemision ,lcImporte ;
			,lccliente ,lctitular ,lccuit ,lcbanco ,lcafecta ;
			,lcLocalidad
	...
	DO WHILE lnCamposLeidos<4
		...
		DO WHILE i + j <= lnCantCampo &&Campos de CsrArti + 1
			...
			ELSE 
				...
				EXIT 
			ENDIF
			lcFecha		= UPPER(LimpiarCadena(IIF(j + i=1,lcCadena,lcFecha)))
			lcFEmision	= UPPER(LimpiarCadena(IIF(j + i=2,lcCadena,lcFEmision)))
			LcCliente		= UPPER(LimpiarCadena(IIF(j + i=3,lcCadena,LcCliente)))
			lcNumero		= UPPER(LimpiarCadena(IIF(j + i=4,lcCadena,lcNumero)))			
			lcBanco		= UPPER(LimpiarCadena(IIF(j + i=5,lcCadena,lcBanco)))
