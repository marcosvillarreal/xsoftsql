...
llok = CargarTabla(lcData,'ProductoPrecio',.t.)
...
lnidlista = RecuperarID('CsrProductoPrecio',Goapp.sucursal10)
...
*!*		lnPeso		= 0
	...
	IF VAL(CsrPrecio.codigo) = VAL(CsrArticulo.codigo) 
		...
		lnprevtaf1	= lnprevta1*(1 + IIF(lnTasa=0,21,10.5)/100)
		lnCosto		= lnCostoSiva*(1 +  IIF(lnTasa=0,21,10.5)/100)
		
		IF lnidlista = 1100000006
			*stop()
		ENDIF 
		
		IF lnCosto < 1000000
			INSERT INTO CsrProductoPrecio (id,idlista,idarticulo,costo,costosiva,margen,prevta,prevtaf);
			VALUES (lnidlista,CsrListaPrecio.id,lnid,lnCosto,lnCostoSiva,lnUtil1,lnprevta1,lnprevta1)
