...
SELECT distinct UPPER(proveedor) as nombre  FROM CsrArticulo INTO CURSOR FsrMarca READWRITE 
...
SELECT distinct UPPER(rubro) as nombre  FROM CsrArticulo INTO CURSOR CsrSeccion READWRITE 
...
SCAN FOR !EOF()
	...
	*	STOP()
	...
	SELECT CsrMarca 
	LOCATE FOR upper(nombre) = upper(CsrArticulo.proveedor)
	IF upper(nombre) = upper(CsrArticulo.proveedor)
		SELECT CsrCtacte
		LOCATE FOR ALLTRIM(cnombre)=ALLTRIM(CsrArticulo.proveedor)
		IF FOUND()
...
*!*		lnPeso		= 0
		...
		*Lista1 Tiene el prescio c/iva, debo componer para atras con un margen del 30%
		lnMargen		= VAL(CsrArticulo.lista1)
		lnPrevta1		= VAL(CsrArticulo.lista2)
