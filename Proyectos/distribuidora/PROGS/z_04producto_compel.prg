...
CREATE CURSOR FsrArticulo (codigo c(8),Articulo c(100),Precio c(20),CodBarra c(20),Codrubro c(50);
	,CodProveedor c(15),CodMarca c(50))

CREATE CURSOR CsrLista (deta01 c(250),deta02 c(250))
		
Oavisar.proceso('S','Abriendo archivos') 

SELECT CsrLista
APPEND FROM  "j:\bic.csv" SDF

lcSeparador =  ";"&&"	"
lcDelimitador = "^"
replace ALL deta01 WITH RTRIM(STRTRAN(deta01,lcSeparador,lcDelimitador))
replace ALL deta02 WITH RTRIM(STRTRAN(deta02,lcSeparador,lcDelimitador))+lcDelimitador
...
*!*		ENDIF 
	...
	IF AT(lcDelimitador,deta01)=lnPrimeraOcurrencia
		leiunarticulo = .t.
		STORE "" TO lcRubro,lcCodigo,lcCosto,lcArticulo
		STORE "" TO lcTipo,lcModelo,lcFecha,lcMarca,lcCodBarra,lcAcarreo,lcCodProveedor
		j = 0
	ELSE
		IF !leiunarticulo
			SKIP 
	...
	DO WHILE lnCamposLeidos<3
		...
		DO WHILE i + j <= lnCantCampo &&Campos de CsrArti + 1
			...
			*lcfecha			= IIF(j + i=1,DTOC(CDateEngSpa(lcCadena)),lcFecha)
			...
			IF VAL(lcCodigo) = 908443  
				stop()
			ENDIF 
			lcArticulo			= UPPER((IIF(j + i=2,lcCadena,lcArticulo)))
			lcCosto				= UPPER((IIF(j + i=3,lcCadena,lcCosto)))
			lcCodBarra		 	= UPPER(IIF(j + i=4,alltrim((STRTRAN(LimpiarCadena(lcCadena),' ',''))),lcCodBarra))
			lcRubro				= UPPER(LimpiarCadena(IIF(j + i=5,lcCadena,lcRubro)))
			lcCodProveedor		= UPPER(LimpiarCadena(IIF(j + i=6,lcCadena,lcCodProveedor)))
			lcMarca				= UPPER(LimpiarCadena(IIF(j + i=7,lcCadena,lcMarca)))
	...
	IF  i+j >= lnCantCampo
		...
		INSERT INTO FsrArticulo (CODIGO,articulo,Codmarca,precio,Codrubro,codbarra,codproveedor);
		VALUES (lcCodigo,lcArticulo,lcMarca,lcCosto,lcrubro,lcCodBarra,lcCodProveedor)
