...
case upper(rtrim(cName)) = "CSRVERSCONFIG"
	CREATE CURSOR CsrVersConfig (sistema c(30), path c(200), grupo c(50), nrosistema i,pathupdateexe c(200), filelogo c(100))
OTHERWISE
	RETURN .f.
ENDCASE
...
IF esvistaprevia
		...
		TRY 
			*stop()
			IF oConfigTermi.FoxyPreviewer = 'FALSE'
			...
			ELSE
				...
				WITH _Screen.oFoxyPreviewer
					...
					lcName = NombreInforme
					* Optional available parameters
					.cTitle = lcName && The preview window title 
					.cPdfTitle = lcName
					.cSaveDefName = lcName
					.cDestFile = lcName
					.nCopies       = 1 && The quantity of copies to be printed
					REPORT FORM (NombreInforme) PREVIEW
				ENDWITH
			ENDIF 
		CATCH TO oError
...
IF !OCAlista.CursorFill()
	IF AERROR(laError) > 0 AND lbCartel 
		lcMensaje = VerErrorSQL(laError[1])
		lcMensaje = "Error al obtener datos:";
						+CHR(13)+"Código Error:" +strtrim(laError[1]);
        				+CHR(13)+"Mensaje al Operador: "+lcMensaje;
        				+CHR(13)+"Mensaje de Error:"+laError[2];
        				+CHR(13)+"Alias:"+lcaliasCursor;
        				+CHR(13)+lccmdSelectCursor
...
CASE UPPER(lcCursor)="CSRFINANCIA"
	Create Cursor CsrFinancia (registro i AUTOINC,numero n(3),nombre c(25),fecha d,afecta n(10,2),idvalor i;
	              ,esclase c(3),detalle c(50),tasa n(6,3),importe n(10,2),cuota n(2),totcuota n(10,2))
	Sele Csrfinancia
ENDCASE 
...
do while salir
   ...
   if at(' ',cadena) >1
      ...
      if at(' ',cadena) <1 &&at('-',cadena) <1
          subca=subca+cadena
          salir = .f.
      endif
   endif
   subca=strtran(subca,'-','')
   if len(subca)<=lenCampo
