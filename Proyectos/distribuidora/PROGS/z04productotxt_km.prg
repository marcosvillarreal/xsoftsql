...
SCAN FOR !EOF()
   ...
   INSERT INTO CsrRubro(id,numero,nombre,idtipovta,idtipoprod,idfuerzavta,switch);
   			 VALUES (lnid,lnnumero,lcnombre,1,1100000001,1100000001,'00000')
   lnid = lnid + 1
	lnCodigo = lnCodigo + 1 
ENDSCAN
...
SCAN FOR !EOF()
	...
	SELECT CsrCtacte
    LOCATE FOR VAL(cnumero)=VAL(CsrAcreedor.proveedor)
    IF FOUND()
    	lnidctacte = Csrctacte.id
    ENDIF

	nCodRubro = VAL(STRTRAN(CsrCtacte.referencia,'/\',''))
	
    SELECT CsrRubro
    LOCATE FOR numero=nCodRubro 
    IF NOT FOUND()
    	GO BOTTOM
    ENDIF 
