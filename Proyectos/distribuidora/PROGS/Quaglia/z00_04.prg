...
FUNCTION LeerClientes_04(cArchivo)

CREATE CURSOR CsrLista (deta01 c(250),deta02 c(250),deta03 c(250) )

CREATE CURSOR CsrDeudor (Codigo c(10),Categoria c(20),Nombre c(70),Direccion c(100),CodLocalidad c(6),Localidad c(50);
	...
	DO WHILE lnCamposLeidos<4
		...
		DO WHILE i + j <= lnCantCampo &&Campos de CsrArti + 1
			...
			*lcCodigo		= UPPER(LimpiarCadena(IIF(j + i=1,lcCadena,lcCodigo)))
			...
			lcTelefono		= UPPER(LimpiarCadena(IIF(j + i=5,lcCadena,lcTelefono)))			
			lcDocumento		= UPPER(LimpiarCadena(IIF(j + i=3,lcCadena,lcDocumento)))
			*lcReferencia	= UPPER(LimpiarCadena(IIF(j + i=10,lcCadena,lcReferencia)))
			*lcVendedor		= UPPER(LimpiarCadena(IIF(j + i=1,lcCadena,lcVendedor)))	
...
FUNCTION LeerArticulos_04(lcArchivo)

CREATE CURSOR CsrLista (deta01 c(250),deta02 c(250),deta03 c(250) )
CREATE CURSOR CsrArticulo (Codigo c(10),Rubro c(50),Nombre c(100),Proveedor c(30);
		...
		,Merma c(10),Margen c(10),CodProveedor c(10),Cuit c(15),CodArticulo c(20);
		,CodMarca c(8), MArca c(50),CodBarra c(30))
...
*!*		IF AT(lcDelimitador,deta01)=lnPrimeraOcurrencia
		...
		STORE "" TO lcLista3,lcLista4,lcPesoKilos,lcMerma, lcMargen
		STORE "" TO lcCodProveedor ,lcCuit ,lcCodArticulo ,lcCodMarca 
		STORE "" TO lcMArca ,lcCodBarra
	...
	DO WHILE lnCamposLeidos<4
		...
		DO WHILE i + j <= lnCantCampo &&Campos de CsrArti + 1
			...
			*lcIDJ			= UPPER(LimpiarCadena(IIF(j + i=1,lcCadena,lcIdJ)))
			lcCodigo		= UPPER(LimpiarCadena(IIF(j + i=1,lcCadena,lcCodigo)))
			lcNombre		= UPPER(LimpiarCadena(IIF(j + i=2,lcCadena,lcNombre)))
			lcCosto			= IIF(j + i=3,lcCadena,lcCosto)
			lcLista1		= IIF(j + i=4,lcCadena,lcLista1) &&Precio c/iva
			lcMargen		= IIF(j + i=5,lcCadena,lcMargen)  &&Utilidad
			lcCodProveedor 	= UPPER(LimpiarCadena(IIF(j + i=6,lcCadena,lcCodProveedor ))) 
			lcProveedor		= UPPER(LimpiarCadena(IIF(j + i=7,lcCadena,lcProveedor))) 
			lcCuit 			= UPPER(LimpiarCadena(IIF(j + i=8,lcCadena,lcCuit )))
			lcCodArticulo 	= UPPER(LimpiarCadena(IIF(j + i=9,lcCadena,lcCodArticulo)))
			lcCodRubro		= UPPER(LimpiarCadena(IIF(j + i=10,lcCadena,lcCodRubro)))
			lcRubro			= UPPER(LimpiarCadena(IIF(j + i=11,lcCadena,lcRubro)))
			lcCodMarca		= UPPER(LimpiarCadena(IIF(j + i=12,lcCadena,lcCodMarca)))
			lcMarca			= UPPER(LimpiarCadena(IIF(j + i=13,lcCadena,lcMarca)))			
			lcAlicuota		= IIF(j + i=14,lcCadena,lcAlicuota) 
			lcUniVenta		= IIF(j + i=15,LEFT(lcCadena,1),lcUniVenta) 
			lcCodBarra		= IIF(j + i=16,lcCadena,lcCodBarra)
