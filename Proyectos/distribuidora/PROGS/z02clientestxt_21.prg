...
SELECT distinct codlocalidad,CAST(0 as numeric(10)) as idlocalidad,ALLTRIM(UPPER(localidad)) as nombre,codpostal ,codprovincia,provincia ,SPACE(30) AS Localidad, SPACE(6) as CPostal;
...
*!*		lnCodProvincia = IIF(VAL(CsrCiudad.codprovincia)=13 ,12,lnCodProvincia ) &&santa fe
	...
	*lnCodProvincia = IIF(lnCodProvincia = 0 ,1,lnCodProvincia )
	...
	ELSE
		LOCATE FOR VAL(cpostal) = 8000 &&Bahia Blanca
		IF id#0
			replace idlocalidad WITH CsrLocalidad.id in CsrCiudad
		ENDIF 
	ENDIF 
...
stop()
sELECT distinct zona as nombre FROM CsrDeudor  INTO CURSOR FsrBarrio READWRITE 

lnid = RecuperarID('CsrBarrio',Goapp.sucursal10)
lnCodigo = 1
...
SCAN FOR !EOF()
   ...
   ENDIF
   lnnumero	= lnCodigo 
   lcnombre	= NombreNi(alltrim(UPPER(FsrBarrio.nombre)))
...
*!*	 	nCodigo			= lnCodigo	
	...
	*GO TOP 
	...
	LOCATE FOR ALLTRIM(nombre)=ALLTRIM(CsrDeudor.zona)
	IF ALLTRIM(nombre)=ALLTRIM(CsrDeudor.zona)
		lnidbarrio = CsrBarrio.id
	ENDIF 
...
INSERT INTO CsrCtacte (id,cnumero,cnombre,cdireccion,cpostal,idlocalidad,idprovincia,ctelefono;
	...
	VALUES (lnid, lcnumero ,'DISTRIBUIDORA GATTARI','','';
	,0,0,'',1,'',0,0,0;
	...
	*	stop()
	...
	cDireDpto	= IIF(LEN(LcDire_Dpto)=0,"","D:"+lcDire_Dpto)
  	...
  	*lcDireccion = RTRIM(UPPER(CsrDeudor.direccion)) + ' ' + ALLTRIM(CsrDeudor.direnro) + ' ' + ALLTRIM(CsrDeudor.direpiso) + ' ' + ALLTRIM(CsrDeudor.diredpto)
  	...
  	IF LEN(ALLTRIM(lcTelefono)) = 0
  		lcTelefono	= LTRIM(CsrAcreedor.telefono2)
  	ELSE
  		IF LEN(ALLTRIM(CsrAcreedor.telefono2)) <> 0
  			lcObserva	= lcObserva + CHR(13) + "TELEFONO: " +LTRIM(CsrAcreedor.telefono2)
  		ENDIF 
  	ENDIF 
