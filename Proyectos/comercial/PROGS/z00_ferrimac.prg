...
*!*		IF AT(lcDelimitador,deta01)=lnPrimeraOcurrencia
		...
		STORE "" TO lcTopeSaldo, lcSaldo, lcObserva 
		j = 0
*!*		ELSE
*!*			IF !leiunarticulo
*!*				SKIP 
...
*!*		ENDIF 
	...
	IF lnpos#0 AND i+j >= lnCantCampo
		...
		*lcCodigo = SUBSTR(lcCodigo,4)
		...
		,CodCateIVA,CodLocalidad,CodProvincia,CodVendedor,idorigen,Saldo,TopeSaldo,Observa) ;
		...
		,VAL(lcCodCateIVA),lcCodLocalidad,lcCodProvincia,lcCodVendedor,VAL(lcIdJ),lcSaldo,lcTopeSaldo,lcObserva)
...
*!*		ENDIF 
	...
	DO WHILE lnCamposLeidos<4
		...
		DO WHILE i + j <= lnCantCampo &&Campos de CsrArti + 1
			...
			lcPreciociva 	= UPPER((IIF(j + i=25,lcCadena,lcPreciosiva )))
			lcPreciosiva 	= UPPER((IIF(j + i=26,lcCadena,lcPreciociva )))
