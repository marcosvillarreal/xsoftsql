...
TEXT TO lcCmd TEXTMERGE NOSHOW 
SELECT Csrparaconfig.*,csrdetanrocaja.nrocaja as nrocaja,csrdetanrocaja.fecdesde as fecdesde,csrdetanrocaja.fechasta as fechasta,Csrdetanrocaja.switch as switchCaja
...
TEXT TO lcCmd TEXTMERGE NOSHOW 
SELECT Csrdetaconta.id as idejercicio,csrdetaconta.ejercicio as ejercicio,csrdetaconta.fecdesde as fecdesde,csrdetaconta.fechasta as fechasta
...
lnidejercicio           = CsrCursor.idejercicio
Goapp.idejercicio       = lnidejercicio
Goapp.ejercicio         = CsrCursor.ejercicio

TEXT TO lcCmd TEXTMERGE NOSHOW 
SELECT MAX(csrdetaconta.ejercicio) as ejercicio FROM detaconta as Csrdetaconta
ENDTEXT 

lok =CrearCursorAdapter("CsrCursor",lcCmd)

TEXT TO lcCmd TEXTMERGE NOSHOW 
SELECT Csrdetaconta.id as idejercicio,csrdetaconta.ejercicio as ejercicio FROM detaconta as Csrdetaconta WHERE ejercicio = <<Csrcursor.ejercicio>>
ENDTEXT 
...
IF LEFT(Goapp.switchperfil,1)="1"
	...
	DO FORM frmejercicio NAME ofrmejercicio LINKED WITH lnidejercicio TO loejercicio
	IF VARTYPE(loejercicio)='O'
		Goapp.idejercicio = loejercicio.id
		Goapp.ejercicio   = loejercicio.ejercicio
		RELEASE loejercicio
	ENDIF 
	IF VARTYPE(ofrmejercicio) = "O"
		RELEASE ofrmejercicio
	ENDIF  
	IF Goapp.idejercicio = 0
		Goapp.idejercicio = lnidejercicio
		Goapp.ejercicio   = Csrparaconfig.ejercicio
	ENDIF 
ENDIF 
