...
llok = CargarTabla(lcData,'ProductoDeta')
...
CREATE CURSOR FsrArticulo (fecha c(10),Rubro c(100),Familia c(100),tipo c(50);
	,Articulo c(100),Marca c(50),Descripcion M,observa c(100))
...
SELECT CsrLista
APPEND FROM  "c:\Servicios.txt" SDF
...
SCAN 
	...
	*Recupero el Familia
	IF LEN(LTRIM(substR(deta01,AT(lcDelimitador,deta01,1)-1)))#0 AND ;
	LEN(LTRIM(SUBSTR(deta01,AT(lcDelimitador,deta01,1)+1,AT(lcDelimitador,deta01,2)-(AT(lcDelimitador,deta01,1)+1))))=0
		lninicio = AT(lcDelimitador,deta01,1)-1
		lcFamilia = LimpiarCadena(SUBSTR(deta01,1,lninicio))
	ENDIF 

	IF AT(lcDelimitador,deta01,2)=AT(lcDelimitador,deta01)+1 OR AT(lcDelimitador,deta01,3)=AT(lcDelimitador,deta01,2)+1
		LOOP 
	ENDIF 
	...
	DO WHILE lnCamposLeidos<3
		...
		DO WHILE i + j <= lnCantCampo &&Campos de CsrArti + 1
			...
			*lcfecha			= IIF(j + i=1,DTOC(CDateEngSpa(lcCadena)),lcFecha)
			lcTipo			= LimpiarCadena(IIF(j + i=2,lcCadena,lcTipo))
			lcModelo		= LimpiarCadena(IIF(j + i=2,lcCadena,lcModelo))
			*lcMarca			= LimpiarCadena(IIF(j + i=4,lcCadena,lcMarca))
			lmDescripcion 	= IIF(j + i=3,lcCadena,lmDescripcion)
	...
	IF lnpos#0 AND i+j >= lnCantCampo
		...
		INSERT INTO FsrArticulo (fecha,tipo,articulo,marca,observa,rubro,familia);
...
SELECT MAX(numero) as codmax FROM CsrRubro INTO CURSOR CsrCodigo READWRITE 

SELECT FsrSeccion
Oavisar.proceso('S','Procesando RUBROS') 
GO top

lnnumero = CsrCodigo.codmax
SCAN FOR !EOF()
    lnnumero	= lnnumero + 1 
    lntipoprod 	= 0
    lntipovta   = 1
...
SELECT MAX(numero) as codmax FROM CsrMArca INTO CURSOR CsrCodigo READWRITE 
SELECT FsrMarca
...
SELECT MAX(numero) as codmax FROM CsrFamilia INTO CURSOR CsrCodigo READWRITE 
SELECT FsrFamilia
...
SELECT MAX(numero) as codmax FROM CsrCategoTipo INTO CURSOR CsrCodigo READWRITE 
...
SELECT MAX(numero) as codmax FROM CsrProducto INTO CURSOR CsrCodigo READWRITE 
...
SCAN FOR !EOF()
	...
	lcnombre 		= NombreNi(ALLTRIM(FsrArticulo.tipo)+' '+alltrim(FsrArticulo.articulo))
	...
	*lccodalfa	 = LTRIM(STR(lnNumero))
	...
	,siinforma,importado,esinsumo,endolar,cotidolar,idfamilia,idcategotipo);
	...
	,lnesinsumo,lnendolar,lncotidolar,lnidfamilia,lnidcategotipo)
