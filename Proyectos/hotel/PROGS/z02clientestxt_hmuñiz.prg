...
,CAST(0 as numeric(10)) as idlocalidad, 'A' as estado;
 ...
 ORDER BY CsrProvincia.nombre, CsrDeudor.localidad;
...
*vista()
SCAN 
	IF "CASEROS"$ALLTRIM(UPPER(CsrCiudad.nombre))
	*	stop()
	ENDIF 
...
IF RECCOUNT('CsrCiudad')#0
	*vista()
	SELECT CsrCiudad
	*CursorAdapterToXML('CsrCiudad',ADDBS(SYS(5)+CURDIR())+"LOCALIDADES.XML" )
