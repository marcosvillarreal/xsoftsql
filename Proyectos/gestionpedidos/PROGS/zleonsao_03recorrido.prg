...
SCAN FOR !EOF()
   ...
   INSERT INTO Csrvendedor (id,numero,nombre,comision,planilla,prevta,estado,lista,idctacte;
   			,acumulavale,idsucursal);
   VALUES (lnid,CsrvendeVie.numero,lcnombre,Csrvendevie.comision,1,lnprevta,lnestado,1,0,0,goapp.idsucursal)
   lnid = lnid + 1

ENDSCAN

lnid = ObtenerID('Fletero')
...
SCAN FOR !EOF()
	...
	lnnumcli=CsrFleteroVie.cliente+20000
	
   	SELECT CsrCtacte
   	LOCATE FOR VAL(cnumero) = lnnumcli
   	lnidctacte = IIF(FOUND() AND VAL(cnumero) = lnnumcli,CsrCtacte.id,0)

   	INSERT INTO Csrfletero (id,numero,nombre,direccion,telefono,tipoflete,idctacte,switch,idsucursal);
   	VALUES (lnid,lnnumero,lcnombre,"","",0,lnidctacte,"00000",goapp.idsucursal)
...
lnid			= ObtenerID('Ruta')
lnidcabeza		= ObtenerID('CabeRuta')
lnidrutavdor	= ObtenerID('RutaVdor')
lnidzonaruta	= ObtenerID('ZonaRuta')
lnidcuerruta	= ObtenerID('CuerRuta')
***
SELECT CsrFuerzaVta
GO TOP 
...
SCAN FOR !EOF()
	...
	SELECT Csrctacte
	LOCATE FOR VAL(cnumero)=CsrRecorrido.cliente+20000
	IF VAL(cnumero) # CsrRecorrido.cliente+20000
		SELECT CsrRecorrido
		LOOP 
	ENDIF 
...
SCAN 
	...
	SELECT Csrctacte
	LOCATE FOR VAL(cnumero)=CsrCliente.numero+20000
    	IF VAL(cnumero) = CsrCliente.numero+20000 AND CsrCtacte.ctalogistica=0
