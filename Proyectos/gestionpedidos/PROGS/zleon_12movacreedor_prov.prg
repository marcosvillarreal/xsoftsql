...
llok = CargarTabla('leon','CabeCpra')
...
llok = CargarTabla('leon','AfeBcocar')
...
,IIF(CsrMoviprov.provincia='R',1100000022,1100000002) as idprovincia;
...
,SUM(TotalPI) as TotalPI,AVG(Tasa) as Tasa;
...
SELECT cliente+10000 as cliente,IIF(provincia='R',1100000022,1100000002) as idprovincia;
,SUM(importeco-decontado) as importe;
...
SELECT CsrProveedores.cliente,CsrProveedores.idprovincia,CAST(0 as numeric(11,2)) as saldo;
...
replace ALL saldo WITH (importe-pago) IN CsrAnterior &&Inveritmos el signo pq es acreedor
...
SCAN 
	...
	SELECT CsrAnterior
	replace saldo WITH (saldo - CsrProv_Saldoant.saldo_ant)*-1 &&Restamos pq el saldo_ant postivo es un credto. y aqui el saldo negativo es  credto
ENDSCAN 
...
,CAST(0 as numeric(11,2)) as saldo,CsrProveedores.idprovincia;
...
replace ALL saldo WITH (importe-pago)*-1 IN CsrSaldosPost &&Inveritmos el signo pq es acreedor
...
SCAN FOR !EOF('CsrAnterior')
	...
	IF  ALLTRIM(cnumero)=ALLTRIM(STR(CsrAnterior.cliente))
		...
		lnidcomproba=44 &&Dbto de Cpra
		lnimporte=CsrAnterior.saldo
		lcclasecomp="B"
		IF lnimporte<0
			lnimporte=lnimporte*-1
			lnsigno=1
			lnidcomproba=7 &&Cdto de Cpra
			lcclasecomp="C"
		ENDIF 
		...
		,"Importación. Compactación Mov Provee.",ldfechasis)
...
*!*			VALUES (lnidcabecpra,lnidmaopera,lnidctacte,lcctacte,lccnombre,lccdireccion,lcctelefono,lccpostal;
*!*			,lnidlocalidad,lnidprovincia,lnidtipoiva,lccuit,ldfecha-1,lnidplanpago,lntotal,lcswitch,lnidcategoria;
*!*	        ,lnsigno,SUBSTR(lcfiscal,1,6),0)
...
*!*			ENDIF
		IF CsrAnterior.TotalPBBA#0 OR CsrAnterior.TotalPBRN#0
			lnidprov = IIF(CsrAnterior.TotalPBBA#0,1100000002,1100000022)
			lnconta	 = IIF(CsrAnterior.TotalPBBA#0,8,32)
...
,tipocomp,fecha_fac,vencimien,IIF(ISNULL(decontado),CAST(0 as numeric(11,2)),decontado)as decontado;
,IIF(ISNULL(saldocomp),CAST(0 as numeric(11,2)),saldocomp)as saldo;
,letra,talonario,numcomp,orden;
FROM CsrMoviprov ;
LEFT JOIN CsrComproba ON CsrMoviprov.tipocomp=CsrComproba.numero;
WHERE fecha_fac=>ldfecha AND estado<>'A' ;
order by fecha_fac,orden into cursor CsrPosterior
...
SCAN  
	...
	IF FOUND()
		*ldfechas=DATETIME(YEAR(CsrPosterior.fecha),month(CsrPosterior.fecha),day(CsrPosterior.fecha),0,0,0)
		...
		ldfecha		= DATETIME(YEAR(CsrPosterior.fecha_fac),month(CsrPosterior.fecha_fac),day(CsrPosterior.fecha_fac),0,0,0)
		...
		,111,111,lniddetanrocaja,lnidcomproba,lcnumero,lcclasecomp,1,1,1,"0000","0","Importación",ldfechasis)
		...
		IF lcorigen$'CPR'	 && si es un pago no grabo el cabepra			
			...
			,lcswitch,lnidcategoria,lnsigno*(-1),lcperiodo,0)
		...
		,0,ldfechavto,lnimporte,"Comprobante Importado Sa",lcperiodo,"0000",lnsigno)
		...
		IF  lnentrega<>0
			lnidmovctacte=lnidmovctacte+1
			INSERT INTO CsrMovctacte (id,idmaopera,fecha,ctacte,idctacte,subnumero,idsubcta,cuota,importe;
			,saldo,entrega,vencimien,total,detalle,pefiscal,switch,signo);
			VALUES (lnidmovctacte,lnidauxmaopera,ldfecha,lcctacte,lnidctacte," ",0,1,lnentrega,lnentrega;
			,0,ldfechavto,lnimporte,"Comprobante Importado En",lcperiodo,"0000",lnsigno*(-1))
		ENDIF 
...
*!*			ENDIF
		IF CsrPosterior.TotalPBBA#0 OR CsrPosterior.TotalPBRN#0
			lnidprov = IIF(CsrPosterior.TotalPBBA#0,1100000002,1100000022)
			lnconta	 = IIF(CsrPosterior.TotalPBBA#0,8,32)
